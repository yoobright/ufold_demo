...
class U_Net_FP(nn.Module):
    def __init__(self, img_ch=17, output_ch=1):
        ...
        self.Conv2 = conv_block(
            ch_in=int(32) + FPNch[0], ch_out=int(64 * CH_FOLD), size=3
        )
        self.Conv3 = conv_block(
            ch_in=int(64 * CH_FOLD) + FPNch[1], ch_out=int(128 * CH_FOLD)
        )
        self.Conv4 = conv_block(
            ch_in=int(128 * CH_FOLD) + FPNch[2], ch_out=int(256 * CH_FOLD)
        )
        self.Conv5 = conv_block(
            ch_in=int(256 * CH_FOLD) + FPNch[3], ch_out=int(512 * CH_FOLD)
        )

        self.Up5 = tp_conv(
            ch_in=int(512 * CH_FOLD) + FPNch[4], ch_out=int(256 * CH_FOLD)
        )
        self.Up_conv5 = conv_block(
            ch_in=int(512 * CH_FOLD) + FPNch[3], ch_out=int(256 * CH_FOLD)
        )

        self.Up4 = tp_conv(ch_in=int(256 * CH_FOLD), ch_out=int(128 * CH_FOLD))
        self.Up_conv4 = conv_block(
            ch_in=int(256 * CH_FOLD) + FPNch[2], ch_out=int(128 * CH_FOLD)
        )

        self.Up3 = tp_conv(ch_in=int(128 * CH_FOLD), ch_out=int(64 * CH_FOLD))
        self.Up_conv3 = conv_block(
            ch_in=int(128 * CH_FOLD) + FPNch[1], ch_out=int(64 * CH_FOLD)
        )
        ...
        self.Conv_1x1 = nn.Conv2d(
            int(32), output_ch, kernel_size=1, stride=1, padding=0
        )
